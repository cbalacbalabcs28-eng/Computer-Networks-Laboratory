    ...
    do
    {
        ...
        switch (choice)
        {
            ...
            case 3:
                ...
                {
                    printf("\nExisting cost between node %d and node %d is: ", u, v);
                    if (cost[u][v] == INF)
                        printf("INF (No direct connection)\n");
                    else
                        printf("%d\n", cost[u][v]);

                    printf("Enter new cost (Use -1 for NO direct connection): ");
                    scanf("%d", &newCost);
                    ...
                    /* Updating both directions assuming an undirected graph */
                    cost[u][v] = newCost;
                    cost[v][u] = newCost;
